import VModal from 'vue-js-modal/dist/ssr.nocss';
import VTooltip from 'v-tooltip';

import * as components from '../src/components';
import * as defaultIcons from '../src/assets/icons';

import translations from '../src/i18n';

const isBrowser = typeof window !== 'undefined';

const ComponentLibrary = {
  install(Vue, options = { icons: {} }) {
    const { icons } = options;

    for (const componentName in components) {
      const component = components[componentName];

      Vue.component(component.name, component);
    }

    Vue.prototype.$icons = {
      ...defaultIcons,
      ...icons,
    };

    Vue.use(VTooltip);
    Vue.use(VModal);

    Vue.prototype.$t = (key, params = {}) => {
      const translate = Vue.prototype.$t || Vue.prototype.t;
      const translation = translate(key, params);

      // First check if translation exists in existing translation system (Nuxt i18n, Inertia, ...)
      // If not, use inherent translations.

      if (translation === key) {
        const locale = document.documentElement.lang.split('-')[0];
        return translations[locale][key];
      }
    };
  },
};

export * from '../src/components';

export { default as tailwindConfig } from '../tailwind.config';
export * as icons from '../src/assets/icons';

export default ComponentLibrary;

if (isBrowser && window.Vue) {
  window.Vue.use(ComponentLibrary);
}
